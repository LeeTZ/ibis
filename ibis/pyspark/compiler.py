...
def compile_selection(t, expr, scope, timecontext, **kwargs):
    op = expr.op()
    arg_timecontexts = [
        adjust_context(node.op(), timecontext) for node in op.selections
    ]
    ...
    if op.sort_keys:
        ...
        res_table = res_table.sort(*sort_cols)

    return filter_by_time_context(res_table, timecontext)
